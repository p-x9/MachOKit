...
extension PrebuiltLoader {
    ...
    internal func _altPath<Cache: _DyldCacheFileRepresentable>(in cache: Cache) -> String? {
        guard layout.altPathOffset != 0 else { return nil }
        guard let offset = cache.fileOffset(
    ...
    internal func _dependentLoaderRefs<Cache: _DyldCacheFileRepresentable>(in cache: Cache) -> DataSequence<LoaderRef>? {
        guard layout.dependentLoaderRefsArrayOffset != 0,
    ...
    internal func _objcBinaryInfo<Cache: _DyldCacheFileRepresentable>(in cache: Cache) -> ObjCBinaryInfo? {
